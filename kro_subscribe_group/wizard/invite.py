...
class Invite(models.TransientModel):
    ...
    _name = 'mail.wizard.invite.group'
    _description = 'Invite group wizard'
    ...
    groups_ids = fields.Many2many('res.groups', string='Group')
    ...
    def add_followers(self):
        ...
        for wizard in self:
            partners_to_sub = self.env['res.partner']
            for gr in wizard.groups_ids:
                partners_to_sub += gr.users.mapped('partner_id')
            Model = self.env[wizard.res_model]
            document = Model.browse(wizard.res_id)

            # filter groups_ids to get the new followers, to avoid sending email to already following partners
            new_partners = partners_to_sub - document.message_partner_ids
