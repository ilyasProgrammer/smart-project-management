odoo.define('website_mail_kro.follow', function (require) {
'use strict';
...
animation.registry.follow = animation.Class.extend({
    ...
    start: function (editable_mode) {
        var self = this;
        this.is_user = false;
        ajax.jsonRpc('/website_mail_kro/is_follower', 'call', {
    ...
    on_click: function () {
        ...
        if (email || this.is_user) {
            ajax.jsonRpc('/website_mail_kro/follow', 'call', {
                'id': +this.$target.data('id'),
    ...
    toggle_subscription: function(follow, email) {
        ...
        if (follow) {
            ...
            this.$target.find(".js_unfollow_btn").addClass("hidden");
               ...
               this.$target.find(".visible").removeClass("hidden");
        ...
        $(".onn").attr("data-follow",'off');
        this.$target.find('input.js_follow_email').attr("disabled", false);
    },
});
